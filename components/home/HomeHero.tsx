...
export function HomeHero({ dealer, className }: HomeHeroProps) {
  ...
  return (
    ...
    <section
      className={cn(
        "relative w-full h-screen overflow-hidden rounded-2xl sm:rounded-3xl",
    ...
    >
      ...
      {/* Atmospheric gradient / soft cyan glow (desktop) */}
      <div
      ...
      <Image
        ...
        sizes="100vw"
        quality={100}
        className="object-cover hidden md:block object-right object-bottom"
      />

      {/* Mobile overlay */}
      <div className="absolute inset-0 bg-gradient-to-b from-black/90 via-black/70 to-black/40 md:hidden" aria-hidden />

      {/* Desktop overlay (keep lighter) */}
      <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/50 to-transparent hidden md:block" aria-hidden />

      {/* Subtle vignette (desktop) */}
      <div
        className="absolute inset-0 rounded-2xl sm:rounded-3xl pointer-events-none hidden md:block"
        aria-hidden
        style={{
